...
public class AminoAcidSet implements Iterable<AminoAcid>
{
    ...
    static {
        locMap = new HashMap<Modification.Location, Modification.Location[]>();
    ...
    private HashMap<Modification.Location, HashMap<Character, AminoAcid[]>> standardResidueAAArrayMap; // std residue -> array of amino acids
    private HashMap<Modification.Location, HashMap<Integer, AminoAcid[]>> nominalMass2aa;    // nominalMass -> array of amino acids
...
//	private ArrayList<AminoAcid> enzymeAAList;
    ...
    {
        aaListMap = new HashMap<Modification.Location, ArrayList<AminoAcid>>();
        standardResidueAAArrayMap = new HashMap<Modification.Location, HashMap<Character, AminoAcid[]>>();
        ...
        while ((s = in.readLine()) != null) {
            ...
            if (s.startsWith(numModsKey)) {
                ...
                if (value.equals("c57")) {
                    ...
                    Modification.Instance modIns = new Modification.Instance(mod, residue, Modification.Location.Anywhere).fixedModification();
                    mods.add(modIns);
            ...
            } else if (s.startsWith(oxidationKey))    // predefined Oxidation
            {
                ...
                for (int i = 0; i < residueStr.length(); i++) {
                    char residue = residueStr.charAt(i);
                    Modification.Instance modIns = new Modification.Instance(mod, residue, Modification.Location.Anywhere);
                    mods.add(modIns);
                }
            ...
            } else if (s.startsWith(pyrogluKey))    // predefined
            {
                ...
                for (int i = 0; i < residueStr.length(); i++) {
                    char residue = residueStr.charAt(i);
                    Modification.Instance modIns = new Modification.Instance(mod, residue, Modification.Location.N_Term);
                    mods.add(modIns);
                }
...
//			aaSet.registerAminoAcid(new AminoAcid('X', "STOP", new Composition(2,6,1,1,0)));
    ...
    public static AminoAcidSet getAminoAcidSet(ArrayList<Modification.Instance> mods,
            ArrayList<AminoAcid> customAminoAcids)
    {
        AminoAcidSet aaSet = new AminoAcidSet();
        for (AminoAcid aa : getStandardAminoAcidSet())
            aaSet.addAminoAcid(aa);
    ...
    {
        return aaListMap.get(Modification.Location.N_Term);
    }

    public ArrayList<AminoAcid> getCTermAAList()
    {
        return aaListMap.get(Modification.Location.C_Term);
    }

    public ArrayList<AminoAcid> getProtNTermAAList()
    {
        return aaListMap.get(Modification.Location.Protein_N_Term);
    }
    ...
    {
        return aaListMap.get(Modification.Location.Anywhere).iterator();
    }
    ...
    public int size()
    {
        return aaListMap.get(Modification.Location.Anywhere).size();
    }
    ...
    {
        return getAminoAcids(Modification.Location.Anywhere, nominalMass);
    }
     ...
     *
     * @param residue the amino acid mass. Use upper case for standard aa (convention). this method is case sensitive.
    ...
    {
        addAminoAcid(aa, Modification.Location.Anywhere);
    }

    // private members
    private void addAminoAcid(AminoAcid aa, Modification.Location location)
    {
        for (Modification.Location loc : locMap.get(location)) {
            // Debug
//			if(aa.isModified())
//				System.out.println("Debug");
    ...
    private void applyModifications(ArrayList<Modification.Instance> mods)
    {
        ...
        Modification.Location[] locArr = new Modification.Location[]{
                Modification.Location.Anywhere,
                Modification.Location.N_Term,
                Modification.Location.C_Term,
                Modification.Location.Protein_N_Term,
                Modification.Location.Protein_C_Term,
        };
        ...
        for (Modification.Instance mod : mods) {
            ...
            if (!containsNTermModification && (location == Modification.Location.N_Term || location == Modification.Location.Protein_N_Term))
                this.containsNTermModification = true;
            if (!containsCTermModification && (location == Modification.Location.C_Term || location == Modification.Location.Protein_C_Term))
    ...
    private void applyFixedMods(HashMap<Modification.Location, ArrayList<Modification.Instance>> fixedMods,
...
//					ModifiedAminoAcid modAA = new ModifiedAminoAcid(aa, mod, modResidue);
    ...
    private void addVariableMods(HashMap<Modification.Location, ArrayList<Modification.Instance>> variableMods,
            Modification.Location location)
    {
        // residue-specific
        for (Modification.Location loc : locMap.get(location)) {
...
//					ModifiedAminoAcid modAA = new ModifiedAminoAcid(targetAA, mod, modResidue);
    ...
    {
        standardResidueAAArrayMap = new HashMap<Modification.Location, HashMap<Character, AminoAcid[]>>();
        nominalMass2aa = new HashMap<Modification.Location, HashMap<Integer, AminoAcid[]>>();
        for (Modification.Location location : Modification.Location.values()) {
        ...
        for (Modification.Location location : aaListMap.keySet()) {
