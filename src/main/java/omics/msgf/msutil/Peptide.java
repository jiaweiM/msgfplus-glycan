...
import omics.msgf.msgf.IntMassFactory;
import omics.msgf.msgf.IntMassFactory.IntMass;
import omics.msgf.msgf.MassListComparator;
...
//	private float nTermModMass = 0f;
    ...
    /**
     * Constructor. Parses sequence string and check for modifications. Not fully implemented!! Examples: QWSYL
     * -17QSVL   QSV+2.12QLK-3
     *
    ...
    public Peptide(String sequence, AminoAcidSet aaSet)
    {
        ...
        for (; index < seqLen; index++) {
            ...
            AminoAcid aa;
            if (isNTerm) {
                aa = aaSet.getAminoAcid(Modification.Location.N_Term, c);
                isNTerm = false;
            } else
     ...
     * Constructor from an ArrayList of AminoAcids.
     *
     * @params aaArray the array of amino acids. added by Kyowon
     */
    ...
    /**
     * This function checks that a given peptide sequence agrees with the mass list. The mass list can expand more than
     * one amino acid.
     *
...
//	}
    ...
    /**
     * Construct an boolean array representing the spectrum. All positions with a theoretical peak will be true.
     *
     * @return
     */
     ...
     *
     * @return true if the last amino acid is triptic and not modified, false otherwise.
     */
    ...
    /**
     * Defines the order of different Sequence objects. The order is determined by the first mass which is greater than
     * the corresponding mass in other (same index) in the other sequence. If there is a tie, the longer Sequence is
     * greater. This is like lexographical ordering.
     *
     * @param other the other Sequence to compare.
     * @return 1 if this Sequence is greater than the other Sequence, -1 if this is smaller, 0 otherwise.
     */
    ...
    /**
     * Matches this peptide against other peptide. If differ only by "I" and "L", returns true, returns false
     * otherwise.
     *
    ...
    /**
     * Convert this peptide into a sequence of compositions in the reversed order. Each amino acid maps to a
  ...
  public int  getNumOfAA(ArrayList<AminoAcid> aaList)
  {
     ...
     * @return a new Sequence object after the slice operation, null if the ranges yield no sequence.
     */
