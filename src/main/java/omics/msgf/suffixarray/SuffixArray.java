package omics.msgf.suffixarray;
...
public class SuffixArray
{
    ...
    /**
     * Constructor that creates a suffixArray file from the given sequence. The name of the suffixArray will have the
     * basePath of the sequence with the suffix array extension attached to it.
     *
     * @param sequence   the sequence object to create the suffix array from.
     * @param suffixFile the path to the precomputed suffix array file. If the file does not exist, write it.
     */
    ...
    /**
     * Tester methods to test all substring can be retrieved. When testing make sure the sequence was created using a 1
     * to 1 mapping function of the alphabet to byte.
     */
    ...
    /**
     * Helper method that initializes the suffixArray object from the file. Initializes indices, leftMiddleLcps,
     * middleRightLcps and neighboringLcps.
     *
    ...
    /**
     * This method translates the suffix array search index into a position of the Adapter (sequence).
     *
     * @param index
     */
    ...
    /**
     * Alternative to search the suffix array in which a MatchSet is return with all the starting positions in the
     * sequence represented by this SuffixArray.
     *
     * @param pattern the ByteSequence to look for. The ByteSequence can be easily translated from the Adapter
     *                sequence.
     ...
     * @return the index returned is the relative position in this suffix array. To get the index in the Adapter
     * sequence, call getPosition.
     */
     ...
     * does a binary search on the suffixArray and returns the starting index of the pattern. A positive number
     * indicates a successful match, while a negative return value means no match.</p>
     * <p>It is very easy to decode the match indices. The return value is
     * guaranteed to be the left-most (smallest) match in the suffix array. Therefore, to retrieve all the matches, one
     * only needs to walk to the right until the sorted suffixes do not match the query.</p>
     * <p>For negative values, it represents the insertion point of the pattern
     * into the suffix array shifted by 1. For example, if the return value is m, then pattern should be inserted at
     * -m-1 and all elements at -m-1, including -m-1 shifted to the right by 1 position. In other words element at -m-1
     * is the first element that is lexographically greater that pattern.</p>
