...
//	private AuditCollection auditCollection;
    ...
    public MZIdentMLGen(SearchParams params, AminoAcidSet aaSet, CompactSuffixArray sa, SpectraAccessor specAcc,
            int ioIndex)
    {
    ...
    /**
     * Original writeResults; after a dependency update, this is no longer writing self-closing tags, resulting in
     * larger mzid files.
     *
     * @param file
     */
    ...
    /**
     * Output results to a StringWriter, then pipe that string through a transform to make empty elements into
    ...
    /**
     * Output results to a temp file, then pipe that file through a transform to make empty elements into self-closing
     * elements
     *
     * @param file
     */
    ...
    public synchronized void addSpectrumIdentificationResults(List<MSGFPlusMatch> resultList)
    {
        for (MSGFPlusMatch mpMatch : resultList) {
            ...
            sir.setId(omics.msgf.mzid.Constants.sirID + specIndex);
            ...
            if (title != null) {
                CvParam cvParam = omics.msgf.mzid.Constants.makeCvParam("MS:1000796", "spectrum title");
            ...
            if (scanNum >= 0) {
                CvParam cvParam = omics.msgf.mzid.Constants.makeCvParam("MS:1001115", "scan number(s)");
            ...
            if (scanStartTime >= 0) {
                CvParam cvParam = omics.msgf.mzid.Constants.makeCvParam("MS:1000016", "scan start time");
                cvParam.setValue(String.valueOf(scanStartTime));
                if (spec.getRtIsSeconds()) {
                    cvParam.setUnitCv(omics.msgf.mzid.Constants.unitCV);
...
//				float theoMz = theoMass/charge;
                ...
                sii.setId(omics.msgf.mzid.Constants.siiID + specIndex + "_" + rank);
...
//				}
                ...
                CvParam rawScoreCV = omics.msgf.mzid.Constants.makeCvParam("MS:1002049", "MS-GF:RawScore");
                rawScoreCV.setValue(String.valueOf(score));
                cvList.add(rawScoreCV);

                CvParam deNovoScoreCV = omics.msgf.mzid.Constants.makeCvParam("MS:1002050", "MS-GF:DeNovoScore");
                deNovoScoreCV.setValue(String.valueOf(match.getDeNovoScore()));
                cvList.add(deNovoScoreCV);

                CvParam specEValueCV = omics.msgf.mzid.Constants.makeCvParam("MS:1002052", "MS-GF:SpecEValue");
                specEValueCV.setValue(specEValueStr);
                cvList.add(specEValueCV);

                CvParam eValueCV = omics.msgf.mzid.Constants.makeCvParam("MS:1002053", "MS-GF:EValue");
                eValueCV.setValue(eValueStr);
                cvList.add(eValueCV);

                if (match.getPSMQValue() != null) {
                    CvParam psmQValueCV = omics.msgf.mzid.Constants.makeCvParam("MS:1002054", "MS-GF:QValue");
                    psmQValueCV.setValue(match.getPSMQValue().toString());
                    cvList.add(psmQValueCV);
                }

                if (match.getPepQValue() != null) {
                    CvParam pepQValueCV = omics.msgf.mzid.Constants.makeCvParam("MS:1002055", "MS-GF:PepQValue");
                    pepQValueCV.setValue(match.getPepQValue().toString());
                    cvList.add(pepQValueCV);
                }

                UserParam isotopeErrorParam = omics.msgf.mzid.Constants.makeUserParam("IsotopeError");
...
//				int isotopeError = NominalMass.toNominalMass(expMass) - NominalMass.toNominalMass(theoMass);
                ...
                if (activationMethodArr != null) {
                    ...
                    UserParam dissociationMethodParam = omics.msgf.mzid.Constants.makeUserParam("AssumedDissociationMethod");
                ...
                if (match.getAdditionalFeatureList() != null) {
                    for (Pair<String, String> feature : match.getAdditionalFeatureList()) {
                        ...
                        UserParam addParam = omics.msgf.mzid.Constants.makeUserParam(name);
...
//		/////////////
//		omics.msgf.msutil.Peptide testPep = aaSet.getPeptide(pepStr);
...
//		System.out.println(pepStr+"->"+(mzidPeptide == null ? null : mzidPeptide.getId()));
        ...
        if (mzidPeptide == null) {
            ...
            omics.msgf.msutil.Peptide peptide = aaSet.getPeptide(pepStr);
            StringBuffer unmodPepStr = new StringBuffer();
            StringBuffer modPepStr = new StringBuffer();
            int location = 1;
            for (AminoAcid aa : peptide) {
                omics.msgf.msutil.Modification.Location loc;
                if (location == 1)
                    loc = omics.msgf.msutil.Modification.Location.N_Term;
                else if (location == peptide.size())
                    loc = omics.msgf.msutil.Modification.Location.C_Term;
                else
                    loc = omics.msgf.msutil.Modification.Location.Anywhere;
                ...
                if (loc == omics.msgf.msutil.Modification.Location.N_Term || loc == omics.msgf.msutil.Modification.Location.C_Term) {
                    List<omics.msgf.msutil.Modification> fixedTermMods = apcGen.getTerminalFixedModifications(aa.getUnmodResidue(), loc);
                    for (omics.msgf.msutil.Modification fixedMod : fixedTermMods) {
                        Modification mod = new Modification();
                        if (loc == omics.msgf.msutil.Modification.Location.N_Term)
                ...
                List<omics.msgf.msutil.Modification> fixedMods = apcGen.getFixedModifications(aa.getUnmodResidue());
                if (fixedMods != null) {
                    for (omics.msgf.msutil.Modification fixedMod : fixedMods) {
                        Modification mod = new Modification();
            ...
            mzidPeptide.setId(omics.msgf.mzid.Constants.pepIDPrefix + modPepStr.toString());
...
//			++startKey;
        ...
        String pepIDNum = peptide.getId().substring(omics.msgf.mzid.Constants.pepIDPrefix.length());
...
//				String pepEvKey = "PepEv"+(index+1)+"_"+length;
                String pepEvKey = omics.msgf.mzid.Constants.pepEvIDPrefix + (index + 1) + "_" + pepIDNum + "_" + start;
//				if(match.isNTermMetCleaved())
//					pepEvKey += "_N"+"_"+startKey;
    ...
    public DBSequence getDBSequence(int protStartIndex)
    {
        ...
        if (dbSeq == null) {
            ...
            if (!isDecoy) {
                CvParam protDescCV = omics.msgf.mzid.Constants.makeCvParam("MS:1001088", "protein description");
    ...
    private void generateCvList()
    {
        ...
        localCvList.add(omics.msgf.mzid.Constants.psiCV);
        localCvList.add(omics.msgf.mzid.Constants.unimodCV);
        localCvList.add(omics.msgf.mzid.Constants.unitCV);
    }
    ...
    private void generateAnalysisSoftwareList()
    {
        ...
        analysisSoftwares.add(omics.msgf.mzid.Constants.msgfPlus);
    }
    ...
    private void generateAnalysisCollection()
    {
        ...
        specIdent.setId(omics.msgf.mzid.Constants.specIdentID);
    ...
    private void initDataCollection()
    {
        ...
        searchDatabase.setId(omics.msgf.mzid.Constants.searchDBID);
        ...
        ffDB.setCvParam(omics.msgf.mzid.Constants.makeCvParam("MS:1001348", "FASTA format"));
        searchDatabase.setFileFormat(ffDB);

        // for decoy
        if (params.useTDA()) {
            searchDatabase.getCvParam().add(omics.msgf.mzid.Constants.makeCvParam("MS:1001197", "DB composition target+decoy"));
            CvParam decoyAccCV = omics.msgf.mzid.Constants.makeCvParam("MS:1001283", "decoy DB accession regexp");
            decoyAccCV.setValue("^" + MSGFPlus.DECOY_PROTEIN_PREFIX);
            searchDatabase.getCvParam().add(decoyAccCV);
            searchDatabase.getCvParam().add(omics.msgf.mzid.Constants.makeCvParam("MS:1001195", "decoy DB type reverse"));
        }
        ...
        spectraData.setId(omics.msgf.mzid.Constants.spectraDataID);
        ...
        ffSpec.setCvParam(omics.msgf.mzid.Constants.makeCvParam(specFileFormat.getPSIAccession(), specFileFormat.getPSIName()));
        ...
        siList.setId(omics.msgf.mzid.Constants.siListID);
        ...
        mzMeasure.setId(omics.msgf.mzid.Constants.measureMzID);
        List<CvParam> cvParamList = mzMeasure.getCvParam();
        cvParamList.add(omics.msgf.mzid.Constants.makeCvParam("MS:1001225", "product ion m/z", omics.msgf.mzid.Constants.psiCV, "MS:1000040", "m/z", omics.msgf.mzid.Constants.psiCV));
