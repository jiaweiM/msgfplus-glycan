...
import omics.msgf.sequences.Sequence;
...
/**
 * An implementation of the Sequence class allowing a fasta file to be used as the database.
 *
 * @author sangtae
 */
public class CompactFastaSequence implements Sequence
{
    ...
    /**
     * map of offset to fasta title (or short title)
     */
    private TreeMap<Integer, String> annotations;

    /**
     * Contents of the protein sequence concatenated into a long string, separate by terminator byte.
     */
    ...
    /**
     * Long representing the time the file was last modified, measured in milliseconds since the epoch (00:00:00 GMT,
     * January 1, 1970)
     */
    ...
    /**
     * Constructor. The alphabet will be created dynamically according from the fasta file.
     *
     * @param filepath the path to the fasta file.
     */
    public CompactFastaSequence(String filepath)
    {
        this(filepath, DBConstants.CAPITAL_LETTERS_26);
    }

    /**
     * Constructor using the specified alphabet set. If there is a letter not in the alphabet, it will be encoded as the
     * INVALID_CHAR_CODE byte.
     *
     * @param filepath the path to the fasta file.
     * @param alphabet the specifications alphabet string. This could take the predefined AminoAcid strings defined in
     *                 this class or customized strings.
     */
    ...
    {
        return getByteAt(position) == DBConstants.TERMINATOR;
    }
...
//		return this.sequence.get((int)position);
     ...
     * Setter method.
     *
     * @param baseFilepath set the baseFilepath for this object. The baseFilepath has no extension.
     */
     ...
     *
     * @param s colon separated string
     */
    private void initializeAlphabet(String s)
    {
        ...
        this.byte2alpha.put(DBConstants.TERMINATOR, DBConstants.TERMINATOR_CHAR);
        this.byte2alpha.put(DBConstants.INVALID_CHAR_CODE, DBConstants.INVALID_CHAR);
        byte value = 2;
        for (byte i = 0; i < tokens.length; i++, value++) {
            for (int j = 0; j < tokens[i].length(); j++) { // for case two char represent the same amino acid
    ...
    /**
     * Read and fasta file and write the processed files given the alphabet
     *
     * @param filepath fasta file path
     * @param alphabet all amino acids separated by colon
     */
        ...
        try {
            ...
            while ((s = in.readLine()) != null) {
                ...
                if (!s.startsWith(">")) {
                    for (int index = 0; index < s.length(); index++) {
                        ...
                        if (encoded != null) {
                            ...
                            seqOut.writeByte(DBConstants.INVALID_CHAR_CODE);
                        }
                    }
                    offset += s.length();
                }
                ...
                // annotation line
                    ...
                    seqOut.writeByte(DBConstants.TERMINATOR);
                    if (annotation != null)
                        metaOut.println(offset + ":" + annotation);

                    offset++; // for terminator
     ...
     *
     * @return meta file {@link FileSignature}
     */
     ...
     *
     * @return seq file {@link FileSignature}
     */
    ...
    /**
     * @return ratio of proteins that sequence is unique
     */
    public float getRatioUniqueProteins()
    {
        int numProteins = 0;
        ArrayList<Integer> proteinLastIndexList = new ArrayList<>(annotations.keySet()); // offset list
        HashMap<Integer, ArrayList<Integer>> lengthProtIndexMap = new HashMap<>();// protein length -> index in protein index
        int fromIndex = 0;
        for (int i = 0; i < proteinLastIndexList.size(); i++) {
            int toIndex = proteinLastIndexList.get(i);
            int length = toIndex - fromIndex; // wrong, should -1
//            System.out.println(length);
            ArrayList<Integer> list = lengthProtIndexMap.computeIfAbsent(length, k -> new ArrayList<>());
            list.add(i);
            fromIndex = toIndex;
        }
    ...
    /**
     * This class represent the state of a file, for test the file consistence.
     */
    private class FileSignature
    {
         ...
         * Constructor
         *
         * @param formatId     file format id
         * @param id           file id
         * @param lastModified last mofified id
         */
