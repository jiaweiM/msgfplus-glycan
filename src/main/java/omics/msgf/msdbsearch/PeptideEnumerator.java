...
public class PeptideEnumerator
{
    ...
    public static void printUsageAndExit(String message)
    {
        ...
        System.out.println("Usage: java -Xmx3500M -cp MSGFPlus.jar omics.msgf.msdbsearch.PeptideEnumerator FastaFile(*.fasta or *.fa) OutputFile");
        System.exit(-1);
    }
...
//		AminoAcidSet aaSet = AminoAcidSet.getStandardAminoAcidSet();
        ...
        CandidatePeptideGrid candidatePepGrid = new CandidatePeptideGrid(aaSet, MAX_PEPTIDE_LENGTH, DBConstants.NUM_VARIANTS_PER_PEPTIDE);
        ...
        for (int bufferIndex = 0; bufferIndex < size; bufferIndex++) {
            ...
            } else if (lcp == 0)    // preceding aa is changed
            {
                ...
                if (precedingAA != DBConstants.TERMINATOR_CHAR && !enzyme.isCleavable(precedingAA)) {
                    i = 0;
                    nnet = 1;
                    if (nnet > 2 - NTT) {
                        continue;
            ...
            for (; i < MAX_PEPTIDE_LENGTH + 1 && index + i < size - 1; i++)    // ith character of a peptide
            {
                ...
                if (!enzyme.isCleavable(residue) && next != DBConstants.TERMINATOR_CHAR) {
                    if (nnet + 1 > 2 - NTT)
                        continue;
                }
