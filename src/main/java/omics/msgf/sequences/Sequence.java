...
/**
 * Interface allowing access to sequence of characters. This abstract both access to elements in the sequence as
 * Characters (in original form) and Bytes (in encoded form).
 *
 * @author jung
 */
public interface Sequence
{
    ...
    /**
     * Return the set of bytes that are valid for sequence. This is the alphabet set in the form of bytes (including the
     * terminator character, but excluding un-encodable characters).
     *
    ...
    /**
     * Get the annotation corresponds to the given position. Annotation can be any string. For example, if the object
     * represented is a fasta file, annotation will be the lines start with ">".
     *
     * @param position the position to query. Annotations are mapped to certain ranges of the sequence indices, so this
     *                 function will return the annotation for the subsequence that falls within this range
     * @return the annotation string corresponds to the given position.
     */
    String getAnnotation(long position);

    /**
     * Get the encoded byte sequence at a given position. No error checking for boundaries is required.
     *
    ...
    /**
     * Retrieve the original character in the sequence at the given position. No error checking for boundaries is
     * required.
     *
    ...
    /**
     * Get the complete entry that corresponds to the given position. A chunk is a contiguous sequence that shares an
     * annotation (e.g. Protein).
     *
     * @param position the position to query.
    ...
    /**
     * Get the complete entry that corresponds to the given fasta entry header. A chunk is a contiguous sequence that
     * shares an annotation (e.g. Protein).
     *
    ...
    /**
     * A quick way to find out whether the given position corresponds to terminating character
     *
    ...
    /**
     * Check that the character at this position is not a terminator and it is in the alphabet.
     *
     ...
     * @return the binary representation if available, but this might fail if the character is not in the alphabet.
     */
    byte toByte(char c);

    /**
     * Take a byte and reverse translate it to the original string representation. Note that some bytes might represent
     * more than one character. An arbitrary character is returned. To find out what the original char was, the
     * getCharAt method should be called instead
     *
     * @param b the byte.
    ...
    /**
     * Returns the starting position of the protein covered by the coordinate given by the parameter
     *
